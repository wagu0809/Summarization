...
class Trainer(object):
    ...
    Args:
            model(:py:class:`onmt.models.model.NMTModel`): translation model
                to train
            train_loss: training loss computation
            valid_loss: training loss computation
            optim: the optimizer responsible for update
            grad_accum_count(int): accumulate gradients this many times.
            report_manager(:obj:`reporter_ext.ReportMgrBase`):
        ...
        while step <= train_steps:
            ...
            for i, batch in enumerate(train_iter):
                ...
                if step % self.save_checkpoint_steps == 0 and self.gpu_rank == 0:
                    self._save(step)
                print(f"batch {i} of epoch {step} is over with loss: {batch_loss}")
